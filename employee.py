...
def exitt():
    sure = messagebox.askyesno("Salir","Seguro que quieres salir?", parent=biller)
    if sure == True:
        biller.destroy()
        root.destroy()
...
class bill_window:
    def __init__(self, top=None):
        ...
        self.button4.configure(text="""Generar""")
        ...
        self.button5.configure(text="""Limpiar""")
        ...
        self.button8.configure(activebackground="palevioletred")
        self.button8.configure(cursor="hand2")
        self.button8.configure(foreground="#ffffff")
        self.button8.configure(background="#d46c91")
            ...
            if(product_name!=""):
                ...
                if product_qty.isdigit()==True:
                    if (stock-int(product_qty))>=0:
                        ...
                        messagebox.showerror("Oops!", "Stock insuficiente, revise la cantidad", parent=biller)
                else:
                    messagebox.showerror("Oops!", "Cantidad inválida", parent=biller)
            else:
                messagebox.showerror("Oops!", "Selecciona un producto.", parent=biller)
        else:
    ...
    def remove_product(self):
        if(self.cart.isEmpty()!=True):
            ...
            messagebox.showerror("Error!", "Añada un producto.", parent=biller)
            ...
            if(cust_name.get()==""):
                ...
                if strr.find('Total')==-1:
                    ...
                    for name, qty in self.cart.dictionary.items():
                        ...
                        db.commit()
                    messagebox.showinfo("Factura completada!", "Factura generada", parent=biller)
